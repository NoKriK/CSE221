...
filesystem cache size manually with the \emph{mdb} command\ref{zfs-evil-tuning}.
We reduced the cache to 32MB.

We then created a file of 64MB and read it 10 times sequentially and read the
size of the file randomly 10 times.
For the random read, we use the urandom function to generate an offset in the
file and we use the lseek system call to move to the position in the file.
